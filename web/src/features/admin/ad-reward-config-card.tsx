...
import type { UpdateAdConfigBody } from '@/types/api';
      ...
      {data && (
        <div style={{ padding: '16px 20px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
            ...
            <NeuField label="Trần lượt/ngày (0 = tắt)">
          ...
          <div style={{ marginTop: 14, background: 'var(--c-yellow)', border: '3px solid var(--c-ink)', borderRadius: 11, padding: '12px 14px', fontSize: 13, fontWeight: 600, lineHeight: 1.6 }}>
            ...
            {' '}(= {formatNumber(Math.round(parsedValue) || 0)}đ × {Number.isFinite(parsedRatio) ? parsedRatio : 0}%). Tối đa{' '}
            <strong>{formatNumber(Math.round(Number(dailyCap)) || 0)} lượt/ngày</strong> ≈{' '}
            {formatNumber(previewGold * (Math.round(Number(dailyCap)) || 0))} Gold/ngày mỗi user.
          </div>
